    ...
    <div class="app-page-container has-padding">
      <div style="display: flex;flex-direction: row;align-items: center;overflow: hidden;margin: 1em 0em;margin: 1.5em 0em 1.7em 0em;">
        ...
        <div style="flex: 1; overflow-x: hidden; padding: 0.3em 0em 0.3em 0.9em;">
          <h1 style="margin: 0; white-space: nowrap; overflow-x: hidden;text-overflow: ellipsis;">Cargar archivo MS Excel</h1>
        </div>
      </div>
      <form action="{{ route('upload.file') }}" method="POST" enctype="multipart/form-data" class="file-upload-form" style="margin-bottom:1.5rem;">
        @csrf
        <label style="margin-bottom: 0.75rem; display:inline-block;">Formato de archivo .XLS o .XLSX.</label><br>
        <input autocomplete="off" style="cursor: pointer; min-width: 25%; overflow: hidden;text-overflow: ellipsis;" class="app-input-text" type="text" id="custom-file-upload" placeholder="Selecciona un fichero para procesar." onclick="document.getElementById('file').click();">
        <input style="width:1px; height:1px;" id="file" type="file" name="file" accept=".xlsx, .xls" onchange="document.getElementById('custom-file-upload').value = this.value;">
        <button class="app-btn app-btn-primary" type="submit">Continuar</button>
      </form>
      <label style="margin-bottom: 0.75rem; display:inline-block;">Recursos y documentación explicativa.</label><br>
      <a href="javascript:void(0)" onclick="showModal('successGen');copyTemplate()" class="app-btn app-btn-primary" style="display: inline-block;line-height: 35px;padding: 0px 16px;margin-right: 12px;white-space: nowrap;margin-bottom: 15px;"><span>Generar Plantilla</span></a>
    </div>
      ...
      <div class="app-alert-modal" aria-modal="true" role="dialog" style="box-shadow: none;">
        <div class="app-alert-header">
          ...
          <div class="app-alert-message">
            <p style="line-height:1.5rem;">Plantilla guardada en <br><b>"Archivos Generados"</b>.</p>
          </div>
        </div>
